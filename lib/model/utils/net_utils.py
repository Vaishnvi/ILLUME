...
class Self_Attn(nn.Module):
    ...
    def __init__(self, in_dim,st):
       ...
       # Construct the conv layers
        self.query_conv = nn.Conv2d(in_channels = in_dim , out_channels = in_dim//8 , kernel_size= 1).cuda(cud)
        self.key_conv = nn.Conv2d(in_channels = in_dim , out_channels = in_dim//8 , kernel_size= 1).cuda(cud)
        self.value_conv = nn.Conv2d(in_channels = in_dim , out_channels = in_dim , kernel_size= 1).cuda(cud)
        
        # Initialize gamma as 0
        self.gamma = nn.Parameter(torch.zeros(1)).cuda(cud)
        self.softmax  = nn.Softmax(dim=-1).cuda(cud)
    ...
    def forward(self,x):
        ...
        print("\n______________________________________",self.st)
        x = x.cuda(cud)
        #domain = domain.cuda(cud)
        #with torch.no_grad():
        ...
        proj_query  = self.query_conv(x).view(m_batchsize, -1, width*height).permute(0,2,1).cuda(cud) # B * N * C
        proj_key =  self.key_conv(x).view(m_batchsize, -1, width*height).cuda(cud) # B * C * N
        energy =  torch.bmm(proj_query, proj_key).cuda(cud) # batch matrix-matrix product
        
        attention = self.softmax(energy).cuda(cud) # B * N * N
        proj_value = self.value_conv(x).view(m_batchsize, -1, width*height).cuda(cud) # B * C * N
        out = torch.bmm(proj_value, attention.permute(0,2,1)).cuda(cud) # batch matrix-matrix product
        out = out.view(m_batchsize,C,width,height).cuda(cud) # B * C * W * H
        
        # Add attention weights onto input
        out = self.gamma*out + x
        del proj_query,proj_key,energy,attention,proj_value,x
        torch.cuda.empty_cache()
